...
import { Menu, CheckCircle2, RotateCw, Target } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // 3. טעינת תוויות (Labels) - הוספתי את זה כאן
        const fetchedLabels = await firestoreService.getLabels();
  ...
  const handleAddTask = async (taskData: Omit<Task, 'id' | 'createdAt' | 'isCompleted' | 'labels'>) => {
    const newTask: Task = {
      ...taskData,
  ...
  const handleDeleteTask = async (id: string) => {
    ...
    const deletePromises = Array.from(tasksToDelete).map(tid => firestoreService.deleteTask(tid));
    await Promise.all(deletePromises);
  };

  const handleAddSubtasks = async (parentId: string, subtaskTitles: string[]) => {
    ...
    await Promise.all(newSubtasks.map(t => firestoreService.addTask(t)));
  };
    ...
    if (activeView.startsWith('label:')) {
      ...
      return (
        <div className="max-w-[800px] mx-auto pb-16">
          ...
          <Inbox tasks={tasks.filter(t => t.labels.includes(labelId))} projects={projects} onToggle={handleToggleTask} onDelete={handleDeleteTask} onAddSubtasks={handleAddSubtasks} onAddTask={handleAddTask} />
        </div>
      )
    }
